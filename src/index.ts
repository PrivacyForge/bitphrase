import { wordList as persianWordList } from "./data/persianWordList";
...
export const encodeUint8ArrayToWord = (bytes: Uint8Array) => {
  ...
  const digits = bigIntegerToDigits(bigintValue, persianWordList.length);
  return digitsToWord(digits);
};

export const decodeWordToUint8Array = (words: string) => {
  const digits = wordToDigit(words);
  const placeValue = generatePlaceValues(digits.length, persianWordList.length);
...
export const isValidEncoding = (text: string) => {
  ...
  for (let i = 0; i < words.length; i++) {
    const validWord = persianWordList.includes(words[i]);
...
const wordToDigit = (text: string) => {
  const digits = text.split(" ").map((word) => persianWordList.indexOf(word));
  return digits;
};
const digitsToWord = (digits: number[]) => {
  return digits.map((x: number) => persianWordList[x]).join(" ");
};
